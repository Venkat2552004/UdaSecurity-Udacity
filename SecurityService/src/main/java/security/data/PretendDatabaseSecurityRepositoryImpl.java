...
public class PretendDatabaseSecurityRepositoryImpl implements SecurityRepository{
    ...
    private security.data.AlarmStatus alarmStatus;
    private security.data.ArmingStatus armingStatus;
    ...
    public PretendDatabaseSecurityRepositoryImpl() {
        try {
            alarmStatus = security.data.AlarmStatus.valueOf(prefs.get(ALARM_STATUS, security.data.AlarmStatus.NO_ALARM.toString()));
            armingStatus = security.data.ArmingStatus.valueOf(prefs.get(ARMING_STATUS, security.data.ArmingStatus.DISARMED.toString()));
    ...
    @Override
    public void setAlarmStatus(security.data.AlarmStatus alarmStatus) {
        this.alarmStatus = alarmStatus;
        prefs.put(ALARM_STATUS, this.alarmStatus.toString());
    }

    @Override
    public void setArmingStatus(security.data.ArmingStatus armingStatus) {
    ...
    @Override
    public security.data.AlarmStatus getAlarmStatus() {
        return alarmStatus;
    }

    @Override
    public security.data.ArmingStatus getArmingStatus() {
        return armingStatus;
    }
}
